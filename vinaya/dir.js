...
for (var i = 0; i < k.length; i++) {
    if (/var ArrLi\s?\=/.test(k[i])) {
        ...
        // 刪除不需要的檔
        k[i] = k[i].replace(',".git"', '').replace(',"index.html"', '').replace(',"dirToIndex.js"', '').replace(',"dir.js"', '').replace(',"x-to-h"', '').replace(',"debug.log"', '').replace(',"_config.yml"', '').replace(',".gitignore"', '').replace(',"vinaya"', '')
    }
}
